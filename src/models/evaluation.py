...
class ModelEvaluator:
    ...
    def cross_validate_model(
        ...
        try:
            ...
            logger.info(
                f"Cross-validation completed. Mean accuracy: {results['cv_mean']:.4f} ± {results['cv_std']:.4f}"
            )
            return results
        ...
        try:
            ...
            for col in numerical_cols:
                if col in current_data.columns:
                    ...
                    mean_drift = (
                        abs(curr_mean - ref_mean) / ref_std if ref_std > 0 else 0
                    )
            ...
            drifted_features = sum(
                1 for v in drift_results.values() if v.get("drift_detected", False)
            )
            ...
            drift_results["overall"] = {
                ...
                "drift_percentage": (
                    drifted_features / total_features if total_features > 0 else 0
                ),
                "drift_severity": (
                    "high"
                    if drifted_features / total_features > 0.5
                    else "medium" if drifted_features / total_features > 0.2 else "low"
                ),
            }

            logger.info(
                f"Data drift detection completed. {drifted_features}/{total_features} features show drift."
            )
    ...
    def generate_evaluation_report(
        self, metrics: Dict[str, Any], model_name: str = "model"
    ) -> str:
        """
    ...
    def track_model_performance(self, model_name: str, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            tracking_file = os.path.join(
                self.results_path, f"{model_name}_performance_history.csv"
            )
