...
def get_kafka_status():
    ...
    try:
        ...
        if kafka_healthy:
            ...
            response = requests.get('http://localhost:8085/api/clusters/okr/topics', timeout=5)
        ...
        for subdir in ['raw', 'processed', 'final', 'models', 'results', 'archive']:
            ...
            if os.path.exists(subdir_path):
                ...
                data_status[subdir] = {
                    ...
                    'size_mb': sum(os.path.getsize(os.path.join(subdir_path, f)) for f in files) / (1024*1024)
                }
            else:
...
def restart_service():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return jsonify({'error': f'Failed to restart service: {result.stderr}'}), 500
