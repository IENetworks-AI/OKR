...
def transform_assets(**kwargs):
    ...
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, "raw", "fixed_assets.json")
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping asset transformation")
            return
        ...
        if "data" not in data or not data["data"]:
            logger.warning("No data found in fixed_assets.json")
            return
        ...
        output_path = os.path.join(BASE_OUTPUT_DIR, "processed", "cleaned_assets.csv")
        df.to_csv(output_path, index=False, encoding="utf-8")
        logger.info(f"Assets transformation completed: {len(df)} records saved to {output_path}")
...
def transform_tools(**kwargs):
    columns_to_keep = ["id", "item_name", "price", "amount", "is_consumable", "is_fixed_asset", "department_id", 
                       "date_of_purchased", "description", "po_id", "store_store_name", "uom_name", "quantity"]
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, "raw", "tools.json")
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping tool transformation")
            return
        ...
        if "data" not in data or not data["data"]:
            logger.warning("No data found in tools.json")
            return
        ...
        output_path = os.path.join(BASE_OUTPUT_DIR, "processed", "cleaned_tools.csv")
        df.to_csv(output_path, index=False, encoding="utf-8")
        logger.info(f"Tools transformation completed: {len(df)} records saved to {output_path}")
...
def transform_inventory(**kwargs):
    columns_to_keep = ["id", "item_name", "price", "amount", "is_consumable", "is_fixed_asset", "department_id", 
                       "date_of_purchased", "description", "po_id", "store_store_name", "uom_name", "quantity"]
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, "raw", "inventory_index.json")
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping inventory transformation")
            return
        ...
        if "data" not in data or not data["data"]:
            logger.warning("No data found in inventory_index.json")
            return
        ...
        output_path = os.path.join(BASE_OUTPUT_DIR, "processed", "cleaned_inventory.csv")
        df.to_csv(output_path, index=False, encoding="utf-8")
        logger.info(f"Inventory transformation completed: {len(df)} records saved to {output_path}")
...
def transform_requested_tool(**kwargs):
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, 'raw', 'requested_tools.json')
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping requested tool transformation")
            return None
        ...
        if "data" not in data or not data["data"]:
            logger.warning("No data found in requested_tools.json")
            return None
            
        flattened_data = flatten_requested_tool(data)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_file = os.path.join(BASE_OUTPUT_DIR, 'processed', f'flattened_requested_{timestamp}.csv')
        ...
        logger.info(f"Requested tool transformation completed: {len(flattened_data)} records saved to {csv_file}")
        return csv_file
    except Exception as e:
...
def transform_requested_inventory(**kwargs):
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, 'raw', 'inventories.json')
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping requested inventory transformation")
            return None
        ...
        if "data" not in data or not data["data"]:
            logger.warning("No data found in inventories.json")
            return None
            
        flattened_data = flatten_requested_inventory(data)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_file = os.path.join(BASE_OUTPUT_DIR, 'processed', f'flattened_inventories_{timestamp}.csv')
        fieldnames = ['id', 'item_name', 'requested_date', 'requested_project_name', 'requested_quantity',
                      'requester_id', 'requester_name', 'store_name', 'tool_id', 'uom_name']
        with open(csv_file, 'w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            ...
            for item in flattened_data:
                writer.writerow(item)
        logger.info(f"Requested inventory transformation completed: {len(flattened_data)} records saved to {csv_file}")
        return csv_file
    except Exception as e:
...
def transform_approved(**kwargs):
    try:
        file_path = os.path.join(BASE_OUTPUT_DIR, 'raw', 'approved.json')
        if not os.path.exists(file_path):
            logger.warning(f"File {file_path} not found, skipping approved transformation")
            return None
        ...
        if not data:
            logger.warning("No data found in approved.json")
            return None
            
        flattened_data = flatten_approved(data)
        if not flattened_data:
            logger.warning("No records with non-null approved_date found in approved.json.")
            return None
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_file = os.path.join(BASE_OUTPUT_DIR, 'processed', f'flattened_approved_{timestamp}.csv')
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as file:
            ...
            for item in flattened_data:
                writer.writerow(item)
        logger.info(f"Approved transformation completed: {len(flattened_data)} records saved to {csv_file}")
        return csv_file
    except Exception as e:
...
def combine_and_produce(**kwargs):
    ...
    try:
        ...
        combined_requested_df.to_csv(os.path.join(BASE_OUTPUT_DIR, 'processed', 'combined_requested.csv'), index=False)
        logger.info("Combined requested files with approved fields saved as 'combined_requested.csv'.")

        # Combine assets, tools, inventory
        assets_df = pd.read_csv(os.path.join(BASE_OUTPUT_DIR, 'processed', 'cleaned_assets.csv'))
        tools_df = pd.read_csv(os.path.join(BASE_OUTPUT_DIR, 'processed', 'cleaned_tools.csv'))
        inventory_df = pd.read_csv(os.path.join(BASE_OUTPUT_DIR, 'processed', 'cleaned_inventory.csv'))
        ...
        combined_all_df.to_csv(os.path.join(BASE_OUTPUT_DIR, 'processed', 'combined_all.csv'), index=False)
