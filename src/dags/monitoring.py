        ...
        if os.path.exists(results_dir):
            ...
            if result_files:
                ...
                context["task_instance"].xcom_push(
                    key="performance_status", value=performance_status
                )
...
def check_data_quality(**context):
    ...
    try:
        ...
        # Store results in XCom
        context["task_instance"].xcom_push(
            key="data_quality_status", value=overall_status
        )
...
def generate_alerts(**context):
    ...
    try:
        ...
        performance_status = ti.xcom_pull(
            task_ids="monitor_model_performance", key="performance_status"
        )
        data_quality_status = ti.xcom_pull(
            task_ids="check_data_quality", key="data_quality_status"
        )
...
generate_alerts_task = PythonOperator(
    task_id="generate_alerts", python_callable=generate_alerts, dag=dag
)
