...
def train_initial_model(**context):
    ...
    try:
        ...
        features_processed, target_processed = preprocessor.preprocess_pipeline(
            features, target
        )
...
def update_model_incrementally(**context):
    ...
    try:
        ...
        data_file = ti.xcom_pull(
            task_ids="collect_streaming_data", key="collected_data_file"
        )
        ...
        if target_col in new_data.columns:
            ...
            new_target = (
                new_data["team_size"] / 20 + new_data["budget"] / 100000 > 0.5
            ).astype(int)
            new_features = new_data

        # Preprocess new data
        new_features_processed, new_target_processed = preprocessor.preprocess_pipeline(
            new_features, new_target
        )

        # Update model
        update_success = trainer.update_model(
            new_features_processed, new_target_processed
        )
        ...
        if update_success:
            ...
            metrics = evaluator.evaluate_model(
                trainer.current_model, new_features_processed, new_target_processed
            )

            # Save evaluation results
            evaluator.save_evaluation_results(
                metrics, f"model_v{trainer.model_version}"
            )

            # Track performance
            evaluator.track_model_performance(
                f"model_v{trainer.model_version}", metrics
            )
        ...
        context["task_instance"].xcom_push(
            key="model_version", value=trainer.model_version
        )
...
def monitor_model_performance(**context):
    ...
    try:
        ...
        ti = context["task_instance"]
        initial_metrics = ti.xcom_pull(
            task_ids="train_initial_model", key="model_metrics"
        )
        update_success = ti.xcom_pull(
            task_ids="update_model_incrementally", key="update_success"
        )
        model_version = ti.xcom_pull(
            task_ids="update_model_incrementally", key="model_version"
        )
        ...
        monitoring_summary = {
            "initial_model_accuracy": (
                initial_metrics.get("accuracy", 0) if initial_metrics else 0
            ),
        ...
        context["task_instance"].xcom_push(
            key="monitoring_summary", value=monitoring_summary
        )
...
train_initial_task = PythonOperator(
    task_id="train_initial_model", python_callable=train_initial_model, dag=dag
)

collect_data_task = PythonOperator(
    task_id="collect_streaming_data", python_callable=collect_streaming_data, dag=dag
)
