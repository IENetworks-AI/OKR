...
def transform_data(**context):
    ...
    try:
        ...
        context["task_instance"].xcom_push(
            key="processed_data_file", value=processed_data_file
        )
        context["task_instance"].xcom_push(
            key="quality_report_file", value=quality_report_file
        )
        context["task_instance"].xcom_push(
            key="processed_data_count", value=len(df_processed)
        )
        context["task_instance"].xcom_push(
            key="quality_score", value=quality_report.get("quality_score", 0)
        )
...
def load_data(**context):
    ...
    try:
        ...
        ti = context["task_instance"]
        processed_data_file = ti.xcom_pull(
            task_ids="transform_data", key="processed_data_file"
        )
...
def validate_pipeline(**context):
    ...
    try:
        ...
        processed_count = ti.xcom_pull(
            task_ids="transform_data", key="processed_data_count"
        )
        ...
        if validation_results["data_loss"] > 0:
            ...
            validation_results["warnings"] = [
                f"Data loss detected: {validation_results['data_loss']} records"
            ]
        ...
        if quality_score and quality_score < 0.8:
            ...
            validation_results["warnings"].append(
                f"Low data quality score: {quality_score:.3f}"
            )
...
validate_task = PythonOperator(
    task_id="validate_pipeline", python_callable=validate_pipeline, dag=dag
)
