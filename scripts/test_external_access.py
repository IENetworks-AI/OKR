...
class ExternalAccessTester:
    ...
    def test_http_service(self, service_name: str, port: int, endpoint: str = "/", expected_status: int = 200) -> bool:
        ...
        try:
            ...
            if response.status_code == expected_status:
                logger.info(f"✓ {service_name} is accessible (Status: {response.status_code})")
                ...
                logger.warning(f"⚠ {service_name} responded with unexpected status: {response.status_code}")
                self.results[service_name] = {
                    'status': 'warning',
                    'url': url,
                    'response_code': response.status_code,
    ...
    def test_tcp_port(self, service_name: str, port: int) -> bool:
        ...
        try:
            ...
            logger.warning(f"⚠ netcat not available, skipping TCP port test for {service_name}")
            return True
        except Exception as e:
    ...
    def test_airflow_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✓ Airflow API is accessible (Status: {response.status_code})")
                ...
                logger.warning(f"⚠ Airflow API responded with status: {response.status_code}")
                self.results['airflow_api'] = {
                    'status': 'warning',
                    'url': url,
                    'response_code': response.status_code,
...
def main():
    ...
    if successful == total:
        ...
        logger.warning("⚠ Some external access tests failed. Check the report for details.")
        return 1
